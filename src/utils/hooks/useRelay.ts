...
export function useRelay() {
  ...
  const connectRelay = useMutation({
    mutationFn: async (relay: NDKRelayUrl, purpose?: 'read' | 'write' | undefined) => {
      ...
      await queryClient.cancelQueries({ queryKey: ['relays', storage.account.pubkey] });
      ...
      queryClient.setQueryData(['relays', storage.account.pubkey], (prev: NDKTag[]) => [
        ...prev,
        ['r', relay, purpose ?? ''],
      ]);
    ...
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ['relays', storage.account.pubkey] });
    },
  });
